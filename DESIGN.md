...
- 容器宽度：正文外壳桌面使用 `max-w-6xl`（1152px）居中；短文案保持 `max-w-2xl`，避免行长失控
- 已登录首页在 `lg` 以上采用双区工作台：左侧 336px 存储策略，右侧自适应上传操作；内容宽，但文本不被拉成宽行
...
- `≥ 1024px`：`max-w-6xl` 双区工作台，存储策略在左、上传操作在右
- `640px-1023px`：单列内容，存储方式两列并排，操作按钮允许分行
- `< 640px`：全宽单列，按钮全宽堆叠，拖拽区 min-height 200px
