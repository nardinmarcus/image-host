...
export default function UploadPanel({
  ...
  return (
    <section className="flex flex-col gap-6" aria-labelledby="upload-title">
      ...
      {!isAuth ? (
        <div className="rounded-3xl border border-teal-100 bg-white p-6 shadow-[0_2px_12px_rgb(15_23_42/0.04)]">
        ...
        <>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div className="flex-1 rounded-xl bg-slate-100/70 px-4 py-3 text-sm text-slate-600">队列中 <strong className="tabular-nums text-slate-900">{pending}</strong> 个文件，共 <strong className="tabular-nums text-slate-900">{totalSizeMB}</strong> MB{successful ? <span className="ml-2 text-teal-700">已完成 {successful}</span> : null}</div>
