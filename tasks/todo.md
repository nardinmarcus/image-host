...
### 阶段 8 · 文件资产与访问洞察 — **完成并已部署**

完整产品规格见 [`docs/admin-insights-spec.md`](../docs/admin-insights-spec.md)。首期目标是让后台回答“资产规模、访问热度、可操作的文件详情和数据完整度”，而不是堆叠无决策价值的图表。

- [x] 概览：资产/KPI、30 天访问趋势、类型与存储分布、热门与待关注资源
- [x] 资源：URL 同步筛选/分页、按 PV/最近访问/体积排序、点穿筛选
- [x] 详情抽屉：预览、元数据、访问摘要、来源、操作与移动端全屏态
- [x] 新上传：记录 `size_bytes`、规范化 MIME/kind 与元数据采集状态
- [x] 历史 R2 补全：以小批量、可恢复任务读取对象头；TG 历史文件明确标记为未采集
- [x] 数据治理：元数据覆盖率、类型不明、长期未访问和异常访问候选
- [x] 验收：指标口径、空状态、范围筛选、移动端及 D1 查询性能符合规格
...
## 阶段 8 验收记录（2026-07-10）

- `npm run lint`、`npm run verify:p0-p1`、`git diff --check` 通过。
- 本地 D1 已应用并验证 `0002_admin_insights.sql`；生产 D1 同步完成，历史上传/访问时间均已转为 ISO8601。
- 生产数据验收：133 个唯一资源、近 7 天 1,283 PV、6 个独立 IP、26 个活跃资源；R2 历史大小已补全 13/13 个唯一资源（共 7.0 MB），TG 历史文件诚实显示为未采集。
- 浏览器验收：7/30 天切换、热门资源详情、KPI 点穿/URL 状态、桌面详情抽屉和 390px 移动端概览/详情通过。
